import { __commonJS, __toESM, require_react } from "./react-C34hXC_j.js";
...
function hydrate(client, dehydratedState, options) {
	...
	queries.forEach(({ queryKey, state, queryHash, meta, promise }) => {
		...
		if (promise) {
			...
			query.fetch(void 0, { initialPromise });
		}
	});
}
...
var require_react_jsx_runtime_development = /* @__PURE__ */ __commonJS({ "node_modules/react/cjs/react-jsx-runtime.development.js": ((exports) => {
	(function() {
		...
		*/
		...
		var jsx$2 = jsxWithValidationDynamic;
...
//#endregion
//#region node_modules/@tanstack/react-query/build/modern/isRestoring.js
...
//#endregion
//#region node_modules/@tanstack/react-query/build/modern/utils.js
...
var ensureSuspenseTimers = (defaultedOptions) => {
	if (defaultedOptions.suspense) {
		if (defaultedOptions.staleTime === void 0) defaultedOptions.staleTime = 1e3;
...
function useQueries({ queries,...options }, queryClient) {
	...
	import_react.useSyncExternalStore(import_react.useCallback((onStoreChange) => isRestoring ? () => void 0 : observer.subscribe(notifyManager.batchCalls(onStoreChange)), [observer, isRestoring]), () => observer.getCurrentResult(), () => observer.getCurrentResult());
	import_react.useEffect(() => {
...
var HydrationBoundary = ({ children, options = {}, state, queryClient }) => {
	const client = useQueryClient(queryClient);
	const [hydrationQueue, setHydrationQueue] = import_react.useState();
		...
		if (state) {
			...
			for (const dehydratedQuery of queries) {
				...
				if (!existingQuery) newQueries.push(dehydratedQuery);
					...
					if (hydrationIsNewer && (!queryAlreadyQueued || dehydratedQuery.state.dataUpdatedAt > queryAlreadyQueued.state.dataUpdatedAt)) existingQueries.push(dehydratedQuery);
				}
			}
			if (newQueries.length > 0) hydrate(client, { queries: newQueries }, optionsRef.current);
			if (existingQueries.length > 0) setHydrationQueue((prev) => prev ? [...prev, ...existingQueries] : existingQueries);
		}
	}, [
		client,
		hydrationQueue,
...
//#endregion
export { CancelledError, HydrationBoundary, InfiniteQueryObserver, IsRestoringProvider, Mutation, MutationCache, MutationObserver, QueriesObserver, Query, QueryCache, QueryClient, QueryClientContext, QueryClientProvider, QueryErrorResetBoundary, QueryObserver, defaultShouldDehydrateMutation, defaultShouldDehydrateQuery, dehydrate, focusManager, hashKey, hydrate, infiniteQueryOptions, isCancelledError, isServer, keepPreviousData, matchMutation, matchQuery, notifyManager, onlineManager, queryOptions, replaceEqualDeep, skipToken, useInfiniteQuery, useIsFetching, useIsMutating, useIsRestoring, useMutation, useMutationState, usePrefetchInfiniteQuery, usePrefetchQuery, useQueries, useQuery, useQueryClient, useQueryErrorResetBoundary, useSuspenseInfiniteQuery, useSuspenseQueries, useSuspenseQuery };
